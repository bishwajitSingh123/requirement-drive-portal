...
def test_verify_account_valid_code(client:Client):
    pending_user  = PendingUser.objects.create(
        email="abc@email.com", verification_code="5555", password="rsndompass"
    )
...
def test_initiate_password_reset_using_registerd_email(client: Client, user_instance):
    ...
    assert len(messages) == 1
    assert messages[0].level_tag == "success"
    assert str(messages[0]) == "Password reset link has been sent to your email."


def test_initiate_password_reset_using_unregisterd_email(client: Client):
    url = reverse('reset_password_via_email')
    request_data = {"email": "notregisterd@email.com"}
    response = client.post(url, request_data)
    assert response.status_code == 302
    assert not Token.objects.filter(user__email=request_data["email"], 
                                    token_type=TokenType.PASSWORD_RESET).first()
    messages = list(get_messages(response.wsgi_request))
    assert len(messages) == 1
    assert messages[0].level_tag == "error"
...
def test_set_new_password_using_valid_reset_token(client: Client, user_instance):
    ...
    reset_token = Token.objects.create(user=user_instance, token_type=TokenType.PASSWORD_RESET,
                                       token="abcd", created_at=datetime.now(timezone.utc))
...
def test_set_new_password_using_invalid_reset_token(client: Client, user_instance):
    ...
    reset_token = Token.objects.create(user=user_instance, token_type=TokenType.PASSWORD_RESET,
                                       token="abcd", created_at=expired_time)
    ...
    assert len(messages) == 1
    assert messages[0].level_tag == "error"
    assert str(messages[0]) == "Invalid or expired link."